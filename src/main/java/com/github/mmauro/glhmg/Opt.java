...
import com.github.mmauro.glhmg.datastruct.Holder;
...
public final class Opt<T> {
	...
	public static String getDefaultValueString(@NotNull Holder<?> defaultValue) {
		if (defaultValue.getValue() == null) {
			return "no value";
		} else if (defaultValue.getValue() instanceof Color) {
			return "#" + Integer.toHexString(((Color) defaultValue.getValue()).getRGB());
		} else {
	...
	public static final class Builder<T> {
		...
		public Opt<T> build() {
			final String desc;
			if (defaultValue != null) {
				desc = "[Optional] " + this.desc + (defaultValue.getValue() == null ? "" : ". Default value is " + Opt.getDefaultValueString(defaultValue));
			} else {
				desc = this.desc;
			}
